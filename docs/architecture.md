...
- `GameCanvas.tsx` now includes a `CameraController` component for smooth following and zoom.
- `Island.tsx` renders terrain, resource nodes and buildings.
- `Island.tsx` uses block-based terrain generation with `TerrainBlock`, `ResourceBlock`, and `BuildingBlock` components.
